...
internal static class SampleRequestDataChangeFieldCatalogLegacy
{
    ...
    private static readonly IReadOnlyDictionary<string, FieldDefinition> Definitions =
        new Dictionary<string, FieldDefinition>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["product.grs"] = Boolean("product.grs", "GRS", x => x.GRS, (x, value) => x.GRS = value),
            ["product.grs_consumer_type"] = GRSConsumerTypeField("product.grs_consumer_type", "Loại GRS", x => x.GRSConsumerType, (x, value) => x.GRSConsumerType = value),
    ...
    private static FieldDefinition GRSConsumerTypeField(
        string code,
        string label,
        Func<Product, GRSConsumerType?> read,
